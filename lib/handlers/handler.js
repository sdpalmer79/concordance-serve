    ...
    getRoot: async (req, res) => {
        try {
            ...
            if (!isValidObjectId(req.params.id)) {
                throw createError('Invalid ObjectId', { httpStatus: 400 });
            }
            const result = await rootCollection.findOne({ _id: new ObjectId(req.params.id) });
            if (!result) {
                throw createError('Word not found', { httpStatus: 404 });
            }
            res.status(200)
    ...
    postRoot: async (req, res) => {
        try {
            ...
            if (req.body.wordIds) {
                ...
                const result = await wordCollection.find({ _id: { $in: req.body.wordIds } }, { projection: { _id: 1 } }).toArray();
            ...
            if (req.body.wordIds) {
                updateObj.$addToSet.wordIds = { $each: req.body.wordIds };
            }

            // Update root
            if (Object.keys(updateObj.$set).length > 0 || Object.keys(updateObj.$addToSet).length > 0) {
                const result = await rootCollection.updateOne({ _id: new ObjectId(req.params.id) }, updateObj);
                if (result.matchedCount === 0) {
